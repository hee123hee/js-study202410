...
/*
    ...
    => reduce는 반복을 실행할 때마다 currentValue를 accumulator에
       return 조건에 맞게 누적
    => reduce는 마지막 accumulator의 리턴값을 리턴
 */
...
/*
    ...
    반복문의 accumulator시작값을 0번 인덱스에서 initialValue로 교채함
    따라서 initialValue를 0으로 두고 return을 a+b;
...
for(const apple of appleBasket) {
    ...
    // 처음 등장했으면 프로퍼티를 생성, 기존에 있던 컬러면 카운트값만 1추가
    if(color in resultObject){ // in은 해당 키가 객체 안에 있는지 확인
        resultObject[color]++;
    } else { // 해당 색깔이 처음 등장한 경우
        resultObject[color] = 1;
    }
}
...
function myReduce(callbackFn, initialValue) {
    ...
    for (let i = startIndex; i < appleBasket.length; i++) {
        ...
        accumulator = callbackFn(accumulator, accumulator);
    }
    return accumulator;
}
